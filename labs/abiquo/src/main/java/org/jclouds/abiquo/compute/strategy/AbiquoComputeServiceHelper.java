...
public class AbiquoComputeServiceHelper {
    ...
    *           The current user.
    * @param enterprise
    *           The enterprise of the current user.
    * @param datacenter
    *           The datacenter of the template.
    * @param template
    *           The template to deploy.
    * @param options
    *           The template options
    * @return The virtual datacenter to be used to deploy the template or
    *         <code>null</code> if none was found and a compatible one could not
    *         be created.
    */
   public VirtualDatacenter getOrCreateVirtualDatacenter(final User user, final Enterprise enterprise,
         final Datacenter datacenter, final VirtualMachineTemplate template, final AbiquoTemplateOptions options) {
      Iterable<VirtualDatacenter> compatibles = findCompatibleVirtualDatacenters(datacenter, template);

      VirtualDatacenter vdc = options.getVirtualDatacenter() == null ? getFirst(compatibles, null) : find(compatibles,
    ...
    */
   public Iterable<VirtualDatacenter> findCompatibleVirtualDatacenters(final Datacenter datacenter,
    ...
    * 
    * @param vm
    *           The virtual machine to configure.
    * @param gatewayNetwork
    *           The network to be used as a gateway.
    * @param ips
    *           The ips to attach to the virtual machine.
    */
   public void configureNetwork(final VirtualMachine vm, @Nullable final Network<?> gatewayNetwork,
         @Nullable final List<Ip<?, ? extends Network<?>>> ips, @Nullable final List<UnmanagedNetwork> unmanagedIps) {
      if (ips != null) {
    ...
    * @return
    */
   private VirtualDatacenter createCompatibleVirtualDatacenter(final User user, final Enterprise enterprise,
         final Datacenter datacenter, final VirtualMachineTemplate template, final String name) {
      PrivateNetwork defaultNetwork = PrivateNetwork.builder(context).name("DefaultNetwork").gateway("192.168.1.1")
            .address("192.168.1.0").mask(24).build();

      VirtualDatacenter vdc = VirtualDatacenter.builder(context, datacenter, enterprise).network(defaultNetwork)
            .build();

      // Find the first hypervisor in the datacenter compatible with the
      // template
      ...
      logger.warn("Could not create a compatible virtual datacenter for template of type %s", template
            .getDiskFormatType().name());

      return null;
   }

   /**
    * Check if the given template type is compatible with the given hypervisor
    * type.
    * 
    * @param template
    ...
    * @return Boolean indicating if the given template type is compatible with
    *         the given hypervisor type.
    */
   private static boolean isTemplateCompatibleWithHypervisor(final VirtualMachineTemplate template,
         final HypervisorType type) {
