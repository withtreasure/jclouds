 ...
 */
@Singleton
public class AbiquoComputeServiceAdapter implements
   ...
   public NodeAndInitialCredentials<VirtualMachine> createNodeWithGroupEncodedIntoName(final String tag,
      ...
      Datacenter datacenter = enterprise.findAllowedDatacenter(DatacenterPredicates.id(Integer.valueOf(template
            .getLocation().getId())));
      ...
      VirtualDatacenter vdc = helper.getOrCreateVirtualDatacenter(user, enterprise, datacenter, virtualMachineTemplate,
            options);
      ...
      // settings if needed
      helper.configureNetwork(vm, options.getGatewayNetwork(),
            options.getIps() == null ? null : Arrays.asList(options.getIps()), options.getUnmanagedIps() == null ? null
      ...
      return new NodeAndInitialCredentials<VirtualMachine>(vm, vm.getId().toString(), template.getImage()
            .getDefaultCredentials());
   }

   @Override
   public Iterable<VirtualMachineTemplate> listHardwareProfiles() {
      // Abiquo does not have the hardwre profiles concept. Users can consume
      // CPU and RAM
   ...
   public VirtualMachine getNode(final String id) {
      // FIXME: Try to avoid calling the cloudService.findVirtualMachine.
